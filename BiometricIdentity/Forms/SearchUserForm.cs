...
using BiometricIdentity.ViewModels;
    ...
    public partial class SearchUserForm : Form
    {
        ...
        private void OnGetBaseTemplateComplete(bool bSuccess, int nRetCode)
        {
            ...
            if (bSuccess)
            {
                ...
                List<UserSnapshot> matchedUsers = new List<UserSnapshot>();

                List<User> users = null;
                try
                {
                    users = _userRepository.AsQueryable()
                ...
                {
                    List<FtrIdentifyRecord> allPrints = new List<FtrIdentifyRecord>();
        ...
        private void ClearDataGridView()
        {
            ...
            {
                ClearDatagrid clear = new ClearDatagrid(ClearDataGridView);
                this.Invoke(clear);
            }
            else
            {
        ...
        private void PopulateDatagridView(List<UserSnapshot> users)
        {
            if (this.InvokeRequired)
            {
                PopulateDatagrid grid = new PopulateDatagrid(PopulateDatagridView);
        ...
        {
            List<FtrIdentifyRecord> records = new List<FtrIdentifyRecord>();
        ...
        private void dataGridUsers_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 5)
            {
                ...
                Thread thread = new Thread(options =>
                {
                    UserRegistrationForm form = new UserRegistrationForm(userId);
                    form.ShowDialog();
                });
                thread.Start();
                this.Close();
